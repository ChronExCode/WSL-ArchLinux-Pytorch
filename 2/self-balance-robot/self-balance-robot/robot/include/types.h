...
struct Config {
    ...
    double pitch_d_alpha    = 0.2;   // D-term low-pass filter (higher = faster response)
    ...
    // Position loop: displacement → target pitch correction
    double pos_kp           = 0.5;
    double pos_ki           = 0.1;
    double pos_kd           = 0.2;

    // Position output limits (asymmetric, degrees)
    // How much the position loop can tilt the robot to correct drift
    double pos_out_max_fwd  = 5.0;   // max forward lean from position correction
    double pos_out_max_bwd  = -5.0;  // max backward lean from position correction

    // Target pitch limits (asymmetric, degrees)
    // Absolute clamp on the final target_pitch (balance_point + offset + pos + throttle)
    double target_pitch_max_fwd = 15.0;   // max forward target pitch
    double target_pitch_max_bwd = -15.0;  // max backward target pitch
    ...
    // Safety: remote control timeout — auto disarm if no control
    // message received for this duration while BALANCING (ms)
...
struct PidDebugData {
    ...
    double pos_disp        = 0;   // current displacement (turns)
    double pos_target      = 0;   // target displacement (turns)
    double pos_out         = 0;   // position PID output (degrees)
    double cmd_left        = 0;   // commanded left motor velocity (turns/s)
    double cmd_right       = 0;   // commanded right motor velocity (turns/s)
};
