...
void TcpServer::parse_command(const std::string& json) {
    ...
    if (type == "control") {
        ...
        partial.pitch_d_alpha = json_get_double(json, "pitch_d_alpha", -1);
        partial.yaw_kp = json_get_double(json, "yaw_kp", -1);
        partial.speed_kp = json_get_double(json, "speed_kp", -1);
        partial.pos_kp = json_get_double(json, "pos_kp", -1);
        partial.pos_ki = json_get_double(json, "pos_ki", -1);
        partial.pos_kd = json_get_double(json, "pos_kd", -1);
        partial.pos_out_max_fwd = json_get_double(json, "pos_out_max_fwd", -999);
        partial.pos_out_max_bwd = json_get_double(json, "pos_out_max_bwd", 999);
        partial.target_pitch_max_fwd = json_get_double(json, "tgt_pitch_max_fwd", -999);
        partial.target_pitch_max_bwd = json_get_double(json, "tgt_pitch_max_bwd", 999);
...
std::string TcpServer::telemetry_json(double pitch, double roll, double yaw,
    ...
    int n = snprintf(buf, sizeof(buf),
        ...
        "\"pos_l\":%.3f,\"pos_r\":%.3f,"
        "\"pos_disp\":%.3f,\"pos_target\":%.3f,\"pos_out\":%.3f,"
        "\"cmd_l\":%.4f,\"cmd_r\":%.4f",
        pitch, roll, yaw, speed, vbus, state, left_vel, right_vel,
        odrive_usb ? "true" : "false", imu_usb ? "true" : "false",
        ...
        pid_dbg.pos_disp, pid_dbg.pos_target, pid_dbg.pos_out,
        pid_dbg.cmd_left, pid_dbg.cmd_right);
    // Append status_msg if non-empty
    if (!status_msg.empty()) {
        n += snprintf(buf + n, sizeof(buf) - n, ",\"msg\":\"%s\"", status_msg.c_str());
    }
