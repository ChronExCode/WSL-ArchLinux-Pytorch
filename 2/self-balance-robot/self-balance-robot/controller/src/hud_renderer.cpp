...
void render_hud(WaylandApp& app, void* shm_data) {
    ...
    // ── Left Panel: PID Tuning ──────────────────────────────
    {
        ...
        if (app.pid_dirty) {
            ...
            cairo_set_font_size(cr, 18);
            cairo_move_to(cr, pp_x + pp_w - 95, pp_y + 22);
        ...
        for (int i = 0; i < n_params; i++) {
            ...
            cairo_set_font_size(cr, 18);
            cairo_move_to(cr, pp_x + 28, row_y + 18);
            ...
            cairo_move_to(cr, pp_x + 200, row_y + 22);
            ...
            if (selected) {
                ...
                cairo_move_to(cr, pp_x + 300, row_y + 20);
    ...
    // ── Position Panel (new, below joystick area) ───────────
    {
        double pp_x = 320;
        double pp_y = cy - 180;
        double pp_w = 220;
    ...
    auto draw_chart = [&](double chart_cx, double chart_cy, double cw, double ch_h,
        ...
        if (app.chart_paused) {
            ...
            cairo_move_to(cr, plot_x + plot_w / 2 - pext.width / 2, chart_cy + 18);
        ...
        // Vertical cursor
        if (app.chart_paused && app.cursor_pos >= 0) {
            double cursor_px = plot_x + (double)app.cursor_pos / (WaylandApp::CHART_LEN - 1) * plot_w;
        ...
        for (auto& [hist2, name2] : traces) {
            ...
            int n2 = hist2->count;
            if (n2 > 0) {
                int idx = (app.chart_paused && app.cursor_pos >= 0 && app.cursor_pos < n2)
                          ? app.cursor_pos : n2 - 1;
