...
void ODriveUsb::emergency_stop() {
    // Best-effort: don't lock io_mtx_ to avoid deadlock during fault
    // Write directly, ignore errors
    // Split into two writes to keep each under 64-byte USB packet size
    ...
    dev_->bulk_write(reinterpret_cast<const uint8_t*>(stop_vel),
                     static_cast<int>(strlen(stop_vel)), 100);
    ...
    dev_->bulk_write(reinterpret_cast<const uint8_t*>(set_idle),
                     static_cast<int>(strlen(set_idle)), 100);
}
