...
struct WaylandApp {
    ...
    ChartHistory ch_pitch_out,ch_left_vel,ch_right_vel,ch_pitch_err,ch_tgt_pitch,ch_flt_pitch,ch_tgt_speed,ch_spd_est,ch_ang_vel,ch_ang_acc,ch_pos_l,ch_pos_r,ch_cmd_l,ch_cmd_r,ch_gyro_x,ch_gyro_y,ch_gyro_z,ch_acc_x,ch_acc_y,ch_acc_z;
    std::string prev_state;
    void push_chart_data(const Telemetry& t){ if(t.state!=prev_state){ if(t.state=="BALANCING") clear_all_charts(); prev_state=t.state;} if(t.state!="BALANCING") return; ch_pitch_out.push(t.pitch_out); ch_left_vel.push(t.left_vel); ch_right_vel.push(t.right_vel); ch_pitch_err.push(t.pitch_err); ch_tgt_pitch.push(t.tgt_pitch); ch_flt_pitch.push(t.flt_pitch); ch_tgt_speed.push(t.tgt_speed); ch_spd_est.push(t.spd_est); ch_ang_vel.push(t.ang_vel); ch_ang_acc.push(t.ang_acc); ch_pos_l.push(t.pos_l); ch_pos_r.push(t.pos_r); ch_cmd_l.push(t.cmd_l); ch_cmd_r.push(t.cmd_r); ch_gyro_x.push(t.gyro_x); ch_gyro_y.push(t.gyro_y); ch_gyro_z.push(t.gyro_z); ch_acc_x.push(t.acc_x); ch_acc_y.push(t.acc_y); ch_acc_z.push(t.acc_z);} 
    void clear_all_charts(){ ch_pitch_out.clear(); ch_left_vel.clear(); ch_right_vel.clear(); ch_pitch_err.clear(); ch_tgt_pitch.clear(); ch_flt_pitch.clear(); ch_tgt_speed.clear(); ch_spd_est.clear(); ch_ang_vel.clear(); ch_ang_acc.clear(); ch_pos_l.clear(); ch_pos_r.clear(); ch_cmd_l.clear(); ch_cmd_r.clear(); ch_gyro_x.clear(); ch_gyro_y.clear(); ch_gyro_z.clear(); ch_acc_x.clear(); ch_acc_y.clear(); ch_acc_z.clear(); }
