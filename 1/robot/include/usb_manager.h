...
public:
    ...
    /// Check if device is being detached (cancel_async in progress)
    bool is_detaching() const { return detaching_.load(); }
    ...
    /// Callback types
    using RxCallback = std::function<void(const uint8_t* data, int length, int status)>;
...
private:
    ...
    // Hotplug event queue — decouple callbacks from libusb event loop
    struct HotplugEvent_t {
        HotplugEvent event;
        uint16_t vid, pid;
    };
    std::mutex hotplug_queue_mtx_;
    std::vector<HotplugEvent_t> hotplug_queue_;
    std::thread hotplug_dispatch_thread_;
    std::condition_variable hotplug_cv_;
    std::atomic<bool> hotplug_dispatch_running_{false};
