...
ReferenceState BalanceController::generate_reference(const RemoteCommand& cmd, const EstimatedState& state, double dt) {
    ...
    if (std::abs(throttle_filtered_) > cfg_.adaptive_balance_cmd_window) {
        target_displacement_turns_ += target_speed_tps_ * dt;
    } else if (std::abs(state.velocity_tps) < cfg_.stop_hold_capture_window_tps) {
        const double dx = state.displacement_turns - target_displacement_turns_;
        const double step = cfg_.target_displacement_catchup_rate * dt;
        if (std::abs(dx) <= step) {
            target_displacement_turns_ = state.displacement_turns;
        } else {
            target_displacement_turns_ += (dx > 0.0) ? step : -step;
        }
    }

    const double speed_accel = (target_speed_tps_ - prev_target_speed_tps_) / std::max(dt, 1e-6);
    const double x_ref_error = target_displacement_turns_ - state.displacement_turns;
    const double theta_bias = balance_point_deg_ + cfg_.pitch_offset + adaptive_trim_deg_;
    const double theta_from_speed = cfg_.speed_to_pitch_ff * target_speed_tps_;
    const double theta_from_accel = cfg_.accel_to_pitch_ff * speed_accel;
    const double theta_from_position = clamp(cfg_.reference_position_gain * x_ref_error, cfg_.pos_out_max_bwd, cfg_.pos_out_max_fwd);
    const double theta_from_velocity_damping = -cfg_.reference_velocity_damping_gain * state.velocity_tps;

    nominal_pitch_deg_ = theta_bias + theta_from_speed + theta_from_accel + theta_from_position + theta_from_velocity_damping;
...
MotorOutput BalanceController::compute_stabilizing_control(const EstimatedState& state, const ReferenceState& ref, const RemoteCommand& cmd, double dt) {
    ...
    const double k_theta = gain_schedule(cfg_.lqr_k_theta, speed_abs);
    const double k_theta_d = gain_schedule(cfg_.lqr_k_theta_d, speed_abs);
    ...
    debug_.pitch_error = ref.constrained_pitch_deg - state.pitch_deg;
    ...
    debug_.pos_disp = state.displacement_turns;
