...
bool MspImuUsb::send_msp_request(MSPCommand cmd) {
    ...
    return dev_->async_write(
        ...
        [cmd](int status, int) {
            if (status != 0) {
                std::cerr << "[MSP-IMU] Async write failed for CMD "
                          << static_cast<int>(cmd) << " status=" << status << "\n";
            }
        },
        20  // 20ms timeout
    );
...
void MspImuUsb::poll_loop(int interval_us) {
    // Request both ATTITUDE and RAW_IMU every cycle (no alternation).
    // This ensures pitch/roll/yaw and gyro/acc are always from the same time window.
    // Interval should be short enough that IMU data rate >> control loop rate.
    // At 1000μs (1kHz), the control loop (200Hz) gets ~5 fresh IMU samples per tick.
    int effective_interval = std::min(interval_us, 1000);  // cap at 1kHz
    std::cout << "[MSP-IMU] Polling started @ "
              << (1000000 / effective_interval) << " Hz (both ATTITUDE+RAW_IMU)\n";
    ...
    while (polling_.load()) {
        ...
        // Request both in quick succession — no alternation
