...
void LIBUSB_CALL UsbDevice::rx_callback_trampoline(libusb_transfer* xfr) {
    ...
    if (xfr->status == LIBUSB_TRANSFER_COMPLETED && xfr->actual_length > 0) {
        if (ctx->callback) {
            ctx->callback(xfr->buffer, xfr->actual_length);
        }
    }
...
void UsbDevice::cancel_async() {
    ...
    // Note: cancelled transfers will be freed in their callbacks
    // after the event loop processes the cancellation.
}
...
int LIBUSB_CALL UsbManager::hotplug_trampoline(libusb_context*,
    ...
    // Fire matching callbacks
    std::lock_guard<std::mutex> lock(mgr->hotplug_mtx_);
    for (auto& reg : mgr->hotplug_regs_) {
        if (reg.vid == desc.idVendor && reg.pid == desc.idProduct) {
            if (reg.callback) {
                reg.callback(ev, desc.idVendor, desc.idProduct);
            }
        }
    }
