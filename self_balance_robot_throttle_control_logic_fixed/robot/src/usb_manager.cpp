...
void LIBUSB_CALL UsbDevice::rx_callback_trampoline(libusb_transfer* xfr) {
    ...
    // Remove this completed transfer before invoking the user callback.
    // The callback may immediately submit a replacement read; keeping the old
    // transfer in active_transfers_ during that re-submit creates races during
    // cancel/detach.
    auto callback = ctx->callback;
    auto* buffer = xfr->buffer;
    int actual_length = xfr->actual_length;
    int status = xfr->status;
    ctx->dev->remove_transfer(xfr);

    if (callback) {
        callback(buffer, actual_length, status);
    }
