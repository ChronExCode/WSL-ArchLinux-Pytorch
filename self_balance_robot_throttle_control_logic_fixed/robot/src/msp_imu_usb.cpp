...
void MspImuUsb::parse_attitude(const uint8_t* payload, int size) {
    ...
    // Fire callback outside data_mtx_ so callbacks may safely call get_imu_data().
    if (cb) {
        cb(copy);
    }
}
...
void MspImuUsb::parse_raw_imu(const uint8_t* payload, int size) {
    ...
    // Do not refresh latest_data_.timestamp here: freshness is based on
    // MSP_ATTITUDE, because pitch/roll/yaw used by the balance loop are
    // only updated by parse_attitude().
}
