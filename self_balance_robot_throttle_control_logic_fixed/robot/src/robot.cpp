...
bool Robot::initialize(const Config& cfg) {
    ...
    tcp_.set_pid_callback([this](const std::string& json) {
        ...
        cfg_.target_speed_rate_limit = getd("target_speed_rate_limit", cfg_.target_speed_rate_limit); cfg_.target_pitch_rate_limit_dps = getd("target_pitch_rate_limit_dps", cfg_.target_pitch_rate_limit_dps); cfg_.target_displacement_catchup_rate = getd("target_displacement_catchup_rate", cfg_.target_displacement_catchup_rate); cfg_.stop_speed_threshold_tps = getd("stop_speed_threshold_tps", cfg_.stop_speed_threshold_tps); cfg_.stop_hold_capture_window_tps = getd("stop_hold_capture_window_tps", cfg_.stop_hold_capture_window_tps); cfg_.reference_position_gain = getd("reference_position_gain", cfg_.reference_position_gain); cfg_.reference_velocity_damping_gain = getd("reference_velocity_damping_gain", cfg_.reference_velocity_damping_gain); cfg_.speed_to_pitch_ff = getd("speed_to_pitch_ff", cfg_.speed_to_pitch_ff); cfg_.accel_to_pitch_ff = getd("accel_to_pitch_ff", cfg_.accel_to_pitch_ff); cfg_.velocity_feedforward_gain = getd("velocity_feedforward_gain", cfg_.velocity_feedforward_gain);
        cfg_.auto_tune_enable = getb("auto_tune_enable", cfg_.auto_tune_enable); cfg_.auto_tune_rate = getd("auto_tune_rate", cfg_.auto_tune_rate); cfg_.auto_tune_min_scale = getd("auto_tune_min_scale", cfg_.auto_tune_min_scale); cfg_.auto_tune_max_scale = getd("auto_tune_max_scale", cfg_.auto_tune_max_scale); cfg_.auto_tune_pitch_error_target_deg = getd("auto_tune_pitch_error_target_deg", cfg_.auto_tune_pitch_error_target_deg); cfg_.auto_tune_speed_error_target_tps = getd("auto_tune_speed_error_target_tps", cfg_.auto_tune_speed_error_target_tps);
        cfg_.friction_comp_tps = getd("friction_comp_tps", cfg_.friction_comp_tps); cfg_.friction_comp_deadband_tps = getd("friction_comp_deadband_tps", cfg_.friction_comp_deadband_tps); cfg_.friction_comp_fade_tps = getd("friction_comp_fade_tps", cfg_.friction_comp_fade_tps);
...
void Robot::handle_state_command(const std::string& cmd) {
    ...
    if (cmd == "arm") {
        ...
        controller_.reset();
        { std::lock_guard<std::mutex> lock(output_mtx_); last_output_ = MotorOutput{}; }
...
void Robot::control_tick(double dt) {
    ...
    // Encoder data is refreshed by encoder_loop() outside this hard real-time-ish
    // control path.  ODrive ASCII reads can block up to their timeout; keeping
    // them out of control_tick() prevents missed 200Hz deadlines.
    ...
    if ((log_counter_++ % static_cast<uint64_t>(std::max(1, cfg_.logging_decimation))) == 0) {
        MotorOutput output_snapshot;
        {
            std::lock_guard<std::mutex> lock(output_mtx_);
            output_snapshot = last_output_;
        }
        logger_.log(std::chrono::duration<double>(std::chrono::steady_clock::now().time_since_epoch()).count(),
                    get_state(), imu_data, cmd, controller_.get_debug_data(), output_snapshot,
                    enc_displacement_.load(std::memory_order_relaxed), enc_velocity_.load(std::memory_order_relaxed));
    }
}

void Robot::encoder_loop() {
    const auto poll_period = std::chrono::milliseconds(10);  // target 100Hz, best effort

    while (running_.load()) {
        auto t0 = std::chrono::steady_clock::now();

        if (odrive_connected_.load() && odrive_.is_connected()) {
            // These four queries may block; this thread owns that latency.
            ...
            enc_displacement_.store((raw_l + raw_r) * 0.5, std::memory_order_relaxed);
        ...
        auto sleep_time = poll_period - std::chrono::duration_cast<std::chrono::milliseconds>(elapsed);
...
void Robot::telemetry_loop() {
    ...
    while (running_.load()) {
        ...
        try {
            ...
            dbg.cmd_left  = output_snapshot.left_velocity;
            dbg.cmd_right = output_snapshot.right_velocity;
...
void Robot::run() {
    ...
    encoder_thread_ = std::thread(&Robot::encoder_loop, this);
    ...
    std::cout << "[Robot] cleanup: join encoder\n";
