...
static void signal_handler(int sig) {
    ...
    if (count == 1) {
        ...
        std::cerr << "\n[Main] Signal " << sig << " received again, forcing exit.\n";
        std::_Exit(128 + sig);
    }
}
...
int main(int argc, char* argv[]) {
    ...
    // Prefer /dev/tty over STDIN so keyboard quit still works when the process
    // is launched through wrappers, pipes, or redirected stdin.
    int key_fd = -1;
    ...
    if (isatty(STDIN_FILENO)) {
        ...
        if (opened_tty_fd >= 0) key_fd = opened_tty_fd;
    }

    struct termios orig_term{}, raw_term{};
    if (key_fd >= 0 && tcgetattr(key_fd, &orig_term) == 0) {
        raw_term = orig_term;
        raw_term.c_lflag &= ~(ICANON | ECHO);
        raw_term.c_cc[VMIN] = 0;
        raw_term.c_cc[VTIME] = 1;  // 100ms timeout
        if (tcsetattr(key_fd, TCSANOW, &raw_term) == 0) {
            restore_term = true;
        }
    } else {
        std::cerr << "[Main] WARNING: no interactive TTY available; q/ESC quit disabled.\n";
    }

    // Keyboard listener thread — press 'q' or ESC to quit.
    // Important: Robot::running_ only becomes true inside robot_instance.run(),
    // so this thread must stay alive before run() starts.
    std::atomic<bool> key_thread_stop{false};
    std::thread key_thread([&]() {
        while (!key_thread_stop.load(std::memory_order_acquire)) {
            if (key_fd < 0) {
                std::this_thread::sleep_for(std::chrono::milliseconds(100));
                continue;
            }
            if (!robot_instance.is_running()) {
                std::this_thread::sleep_for(std::chrono::milliseconds(50));
                continue;
            }
            char ch = 0;
            ssize_t n = read(key_fd, &ch, 1);
            if (n == 1) {
                ...
                std::cerr << "[Main] Keyboard read failed: " << strerror(errno) << "\n";
                break;
            }
        }
    });
    ...
    key_thread_stop.store(true, std::memory_order_release);
    if (key_thread.joinable()) key_thread.join();

    // Restore terminal
    if (restore_term) tcsetattr(key_fd, TCSANOW, &orig_term);
    if (opened_tty_fd >= 0) close(opened_tty_fd);
