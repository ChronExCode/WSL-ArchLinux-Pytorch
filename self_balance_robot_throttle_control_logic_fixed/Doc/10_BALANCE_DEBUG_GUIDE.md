...
平衡车定义，向前运动，速度为正。平衡车向前倾 pitch为正
...
## 13. Refined field-debug checklist from code review

### 13.1 First bring-up: disable complexity

For the first stable stand-up attempt, use the simplest reliable path:
...
```json
{
  ...
  "auto_lqr_enable": 0.0,
  "max_velocity": 0.4,
  "target_pitch_rate_limit_dps": 40.0,
  "target_speed_rate_limit": 1.0
}
```

Then re-enable features one at a time: complementary-filter/manual feedback, EKF, auto-LQR, then NMPC.

### 13.2 Sign test before wheels leave the ground

| Test | Expected result |
|---|---|
| Tilt body forward | `pitch`, `flt_pitch`, and `theta_err` move consistently. |
| Command small positive velocity | both wheels spin in the intended forward direction. |
| Rotate wheels forward by hand | `pos_l`, `pos_r`, `pos_disp`, and `spd_est` are positive. |
| Tilt forward while balancing logic is active | `base_cmd` drives wheels in the direction that chases the falling body. |

If any row is wrong, fix signs before tuning gains.

### 13.3 If arm enters `CALIBRATING` and stalls

Check, in order: `imu_data.valid`, pitch within ±20 deg, `clear_all_errors()`, `set_velocity_control(0/1)`, `set_closed_loop(0/1)`, then `is_closed_loop(0/1)`. Add timestamps around every ODrive call in `init_odrive_motors()` if the stall point is unclear.

### 13.4 If q/ESC prints shutdown then hangs

Shutdown path:

```text
q/ESC → Robot::shutdown() → run loop exits → stop motors → stop IMU polling → join telemetry → join reconnect → stop logger → stop TCP → detach USB → shutdown USB manager
```

If the last line is `cleanup: stop tcp`, inspect `TcpServer::stop()` and its accept/receive threads. Typical causes are blocking `accept()`, blocking `recv()`, send mutex held while closing sockets, or callback/thread dependency cycles. The fix pattern is: set `running_ = false`, call `shutdown(fd, SHUT_RDWR)` on server and client sockets, close them, then join threads.

If the last line is `join telemetry`, inspect ODrive query timeouts and make sure telemetry does not start a new `get_vbus_voltage()` query after shutdown begins.

### 13.5 If it falls immediately after entering `BALANCING`

1. Set throttle and steering to zero.
2. Reduce `max_velocity`.
3. Disable NMPC and auto-LQR.
4. Confirm `balance_point` equals measured pitch at arm.
5. Watch `theta_err` and `base_cmd` while manually tilting a few degrees.
6. If `base_cmd` direction is wrong, fix sign conventions; do not compensate with larger gains.
7. If direction is right but too weak, increase `lqr_k_theta` first.
8. If it overshoots, increase damping through `lqr_k_theta_d` or reduce `lqr_k_theta`.

### 13.6 If it balances but slowly drives away

Increase `lqr_k_x` if it does not return to the hold point. Increase `lqr_k_v` if it rolls through the hold point. Reduce `reference_position_gain` if target pitch becomes too aggressive. Confirm `target_displacement_turns_` captures actual position only inside the stop window.

### 13.7 If it jitters while almost upright

Likely causes: pitch-rate noise, aggressive ODrive velocity loop, encoder query jitter, `lqr_k_theta_d` too high, or `comp_filter_alpha` too high. Log `gyro_y` and `spd_est` at rest, then temporarily reduce `lqr_k_theta_d` by 30% and compare.

---

## 调试记录：反馈符号修正

本次调试以当前代码包为准，旧代码包和旧修改不再作为依据。
...
向前运动：velocity > 0
车体向前倾：pitch > 0
控制器内部倾角误差：theta_error = pitch - pitch_ref
pitch_error debug 字段：pitch_error = theta_error
```

实车验证结论：原来的 pitch 反馈方向是反的。按上述定义，当车体向前倾时：
...
为了追上车体并把车扶回平衡，轮子应向前运动，因此基础速度命令应为正：

```text
base_velocity > 0
```

所以手动 LQR/PID 分支中的 pitch、pitch-rate、积分反馈符号应为：

```text
fb_theta   = +k_theta   * theta_error
fb_theta_d = +k_theta_d * theta_d_error
i_term     = +k_i       * integral(theta_error)
```
...
验证方法：关闭 NMPC 和 auto LQR，仅验证基础反馈方向。扶住车体向前压车，如果 `theta_error > 0`，应观察到：

```text
fb_theta > 0
base_cmd > 0
cmd_left / cmd_right 在无 steering 时同为正或接近同为正
```

如果此时输出仍为负，优先检查 IMU pitch 方向、ODrive 轴向、左右轮速度正方向是否被再次取反。
