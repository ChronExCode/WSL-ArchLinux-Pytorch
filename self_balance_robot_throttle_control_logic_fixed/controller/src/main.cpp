// 软件设计注释：controller 主程序入口，负责 Wayland 界面、MSI Claw 输入、TCP 通信与参数面板交互，不负责底层平衡闭环。
// ═══════════════════════════════════════════════════════════════
...
// main: controller 总入口。启动顺序可理解为：参数恢复 -> 手柄准备 -> TCP 连接 ->
// Wayland 窗口建立 -> 事件循环和遥测/命令收发。
int main(int argc, char* argv[]) {
    ...
    // Resolve ~/self-balance-robot/controller
    {
        ...
        if (home) {
            app.config_dir = std::string(home) + "/.config";
        } else {
            app.config_dir = "./";
        }
    }
        ...
        while (tcp_running.load() && app.running) {
            if (!app.tcp.is_connected()) {
                ...
                if (app.tcp.connect(app.server_ip, port)) {
                    ...
                    app.tcp.send_line(app.serialize_param_json());
            ...
            for (auto& line : lines) {
                ...
                if (type == "telemetry") {
                    ...
                    app.telem.actual_pitch = json_parse_double(line, "actual_pitch");
                    app.telem.flt_pitch = json_parse_double(line, "flt_pitch");
                    app.telem.flt_pitch_rate = json_parse_double(line, "flt_pitch_rate");
                    ...
                    app.telem.flt_vel   = json_parse_double(line, "flt_vel");
                    ...
                    app.telem.obs_disp  = json_parse_double(line, "obs_disp");
                ...
                if (events[i].events & EPOLLIN) {
                    ...
                    for (int j = 0; j < count; j++) {
                        ...
                        if (event_type == EV_KEY && event_code == MENU_BUTTON && event_value == 1) {
                            ...
                            app.armed = true;
                            app.cursor_pos = app.clamp_cursor_pos_to_data();
                            std::cout << "[Cmd] ARM\n";
                        }
                        ...
                        // LT/RT → move chart cursor (independent of pause)
                        // Track press state for simultaneous detection
                        static bool lt_pressed = false, rt_pressed = false;
                        if (event_type == EV_KEY && event_code == 16) {
                            lt_pressed = (event_value == 1);
                        }
                        if (event_type == EV_KEY && event_code == 19) {
                            rt_pressed = (event_value == 1);
                        }

                        if (event_type == EV_KEY && event_value == 1) {
                            // LT+RT simultaneous → reset cursor to middle
                            if ((event_code == 16 || event_code == 19) && lt_pressed && rt_pressed) {
                                app.cursor_pos = app.clamp_cursor_pos_to_data();
                                app.cursor_visible = true;
                                app.cursor_last_active = std::chrono::steady_clock::now();
                            }
                            // LT (code 16) alone → cursor left
                            else if (event_code == 16) {
                                if (app.cursor_pos < 0) app.cursor_pos = app.clamp_cursor_pos_to_data();
                                if (app.cursor_pos > 0) app.cursor_pos--;
                                app.cursor_visible = true;
                                app.cursor_last_active = std::chrono::steady_clock::now();
                            }
                            // RT (code 19) alone → cursor right
                            else if (event_code == 19) {
                                if (app.cursor_pos < 0) app.cursor_pos = app.clamp_cursor_pos_to_data();
                                const int max_cursor = std::max(0, app.chart_sample_count() - 1);
                                if (app.cursor_pos < max_cursor) app.cursor_pos++;
                                app.cursor_visible = true;
                                app.cursor_last_active = std::chrono::steady_clock::now();
                            }
                        }
                        ...
                        if (auto* p = app.current_pid_param()) {
                        ...
                        if (event_type == EV_KEY && event_code == LB_BUTTON && event_value == 1) {
                            if (!app.param_groups.empty()) {
                                if (app.pid_group == 0) {
                                    app.pid_group = static_cast<int>(app.param_groups.size()) - 1;
                                } else {
                                    app.pid_group--;
                                }
                                app.pid_sel = 0;
                                app.clamp_selection();
                                std::cout << "[PID] Group: " << app.current_group_name() << "\n";
                            }
                        } else if (event_type == EV_KEY && event_code == RB_BUTTON && event_value == 1) {
                            if (!app.param_groups.empty()) {
                                app.pid_group = (app.pid_group + 1) % static_cast<int>(app.param_groups.size());
                                app.pid_sel = 0;
                                app.clamp_selection();
                                std::cout << "[PID] Group: " << app.current_group_name() << "\n";
                            }
                        }
                        // Y → send all parameters
    ...
    while (app.running) {
        ...
        if (app.configured) {
            ...
            if (app.cursor_visible) {
                auto elapsed = std::chrono::steady_clock::now() - app.cursor_last_active;
