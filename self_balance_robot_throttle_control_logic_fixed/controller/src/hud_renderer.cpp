...
void render_hud(WaylandApp& app, void* shm_data) {
    ...
    TelItem items[] = {
        ...
        {"speed",      {}, 0.3, 0.9, 0.5},
        {"vbus",       {}, 0.9, 0.6, 0.3},
        {"left_vel",   {}, 0.5, 0.7, 0.9},
        {"right_vel",  {}, 0.5, 0.7, 0.9},
        {"speed_limit",{}, 0.7, 0.3, 0.9},
        {"armed",      {}, 0.9, 0.3, 0.3},
    };
    ...
    // ── Position Panel (right of PID panel) ───────────────────
    {
        ...
        PosItem pos_items[] = {
            {"pos_disp",   telem.pos_disp},
            {"pos_target", telem.pos_target},
            {"pos_out",    telem.pos_out},
        };
    ...
    auto draw_chart = [&](double chart_cx, double chart_cy, double cw, double ch_h,
        ...
        double margin_l = 280;  // left margin for labels and cursor/latest values
        ...
        for (auto& [hist2, name2] : traces) {
            ...
            cairo_stroke(cr);
            // Label on the left, value immediately to its right.
            // Cursor visible or pause: show cursor sample; otherwise show latest sample.
            int n2 = hist2->count;
            bool show_cursor_value = (app.chart_paused || app.cursor_visible) &&
                                     effective_cursor >= 0 && effective_cursor < n2;
            int idx = (n2 > 0) ? (show_cursor_value ? effective_cursor : n2 - 1) : -1;

            cairo_set_source_rgba(cr, 0.6, 0.7, 0.8, 0.85);
            cairo_move_to(cr, chart_cx + 24, ly);
            cairo_show_text(cr, name2);

            if (idx >= 0) {
                char value_buf[32];
                snprintf(value_buf, sizeof(value_buf), "% .3f", hist2->get(idx));
                cairo_set_source_rgba(cr, lr, lg, lb, 0.95);
                cairo_move_to(cr, chart_cx + 160, ly);
                cairo_show_text(cr, value_buf);
            }
            legend_idx2++;
        }
    };
    ...
    draw_chart(chart_x, chart_y0 - chart_h - chart_gap, chart_w, chart_h,
        "IMU",
        {{&app.ch_gyro_x, "gyro_x"},
         {&app.ch_gyro_y, "gyro_y"},
         {&app.ch_gyro_z, "gyro_z"},
         {&app.ch_acc_x, "acc_x"},
         {&app.ch_acc_y, "acc_y"},
         {&app.ch_acc_z, "acc_z"}},
        {{0.9, 0.3, 0.3},
    ...
    draw_chart(chart_x, chart_y0, chart_w, chart_h,
        "PITCH (\xC2\xB0)",
        {{&app.ch_tgt_pitch, "tgt_pitch"},
         {&app.ch_actual_pitch, "actual_pitch"},
         {&app.ch_flt_pitch, "flt_pitch"},
         {&app.ch_flt_pitch_rate, "flt_pitch_rate"},
         {&app.ch_pitch_err, "pitch_err"}},
        {{0.9, 0.5, 0.2},
         {0.2, 0.9, 0.4},
         {0.3, 0.6, 0.9},
         {0.9, 0.9, 0.3},
    ...
    draw_chart(chart_x, chart_y0 + chart_h + chart_gap, chart_w, chart_h,
        "MOTOR VEL (t/s)",
        {{&app.ch_pitch_out, "pitch_out"},
         {&app.ch_cmd_l, "cmd_l"},
         {&app.ch_cmd_r, "cmd_r"},
         {&app.ch_left_vel, "left_vel"},
         {&app.ch_right_vel, "right_vel"},
         {&app.ch_flt_vel, "flt_vel"}},
        {{0.9, 0.9, 0.3},
         {0.2, 0.8, 0.4},
         {0.8, 0.4, 0.2},
         {0.3, 0.6, 0.9},
    ...
    draw_chart(chart_x, chart_y0 + 2 * (chart_h + chart_gap), chart_w, chart_h,
        ...
        {{&app.ch_pos_l, "pos_l"},
         {&app.ch_pos_r, "pos_r"},
         {&app.ch_pos_disp, "pos_disp"},
         {&app.ch_obs_disp, "obs_disp"}},
        {{0.3, 0.6, 0.9},
         {0.9, 0.3, 0.9},
         {0.9, 0.9, 0.3},
         {0.2, 0.9, 0.4}},
