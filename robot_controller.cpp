...
class GamepadReader {
	...
	public:
    	...
    	int add_fd_to_epoll(int epollfd, const char *devpath) {
...
public:
    ...
    bool open1(const std::string& device) {
...
static void render_hud(WaylandApp& app) {
    ...
    double throttle = 0.5;//-app.gamepad.axis(app.axis_throttle);  // Y inverted
    double steering = 0.3;//app.gamepad.axis(app.axis_steering);FixME
    double lt = 0.2;//app.gamepad.axis(app.axis_speed_lt);
    double rt = 0.1;//app.gamepad.axis(app.axis_speed_rt);
...
*/
    ...
    if (app.joy_map_mode) {
        ...
        if (joy_device == "auto") {
            ...
            opened = app.gamepad.epollfd_open();
        }
        if (!opened) {
        ...
        while (true) {
        	...
        	for (int i = 0; i < nfds; ++i) {
            	if (events[i].events & EPOLLIN) {
                	//handle_input_fd(events[i].data.fd); FixME
            	}
...
//            app.gamepad.poll(); FixME
    ...
    std::thread cmd_thread([&]() {
        ...
        while (tcp_running.load()) {
     //FixMe       app.gamepad.poll();

            if (app.tcp.is_connected()) {
                // Read joystick
                double throttle = 0.3;//-app.gamepad.axis(app.axis_throttle);
                double steering = 0.3;//app.gamepad.axis(app.axis_steering);
                double lt = 0.3;//app.gamepad.axis(app.axis_speed_lt);
                double rt = 0.3;//app.gamepad.axis(app.axis_speed_rt);
                double speed_limit = 1.0 - (lt + rt) * 0.5;
                if (speed_limit < 0.1) speed_limit = 0.1;
                ...
                bool arm_btn = 1;//app.gamepad.button(app.btn_arm);
                bool estop_btn = 1;//app.gamepad.button(app.btn_estop);
                bool disarm_btn = 1;//app.gamepad.button(app.btn_disarm);FixMe
                ...
                // ── X (30): Toggle chart pause ──────────────
                static bool prev_x_pause = false;
                bool x_btn = app.gamepad.button(30);  // X (BTN_SOUTH)
                ...
                int dpad_y = 20;//(int)std::round(app.gamepad.axis(ABS_HAT0Y)); FixME
                int dpad_x = 20;//(int)std::round(app.gamepad.axis(ABS_HAT0X));
